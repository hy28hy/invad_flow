...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y=None, return_tokens=False, **kwargs):
        ...
        # Optional prototype/context conditioning.  Historical InvAD calls pass
        # context=None, so this is exactly backward compatible.  The experiment
        # runner uses Denoiser.cls_embed as a learned class prototype and feeds
        # it here, directly into every adaLN block (no detached auxiliary head).
        context = kwargs.get('context', None)
        if context is not None:
            if context.ndim == 3:
                context = context.mean(dim=1)
            if context.shape != cond.shape:
                raise ValueError(
                    f'context must have shape {tuple(cond.shape)}, got {tuple(context.shape)}')
            cond = cond + context.to(dtype=cond.dtype)
        
        # Research hook used by CNRA-DiT.  It is deliberately parameter-free
        # and disabled by default, so historical checkpoints and inference are
        # bit-for-bit unchanged.  Keeping the tensors (rather than detached
        # copies) lets an external normality projector backpropagate directly
        # into selected DiT blocks.
        capture = set(getattr(self, 'capture_hidden_indices', ()))
        captured = []
        for block_idx, block in enumerate(self.blocks):
            x = block(x, cond)  # (B, N, C)
            if block_idx in capture:
                captured.append(x)
        self.last_hidden_states = captured
